...
const versions = Object.fromEntries(['energy.js', 'app-shell.js', 'app-shell.css'].map(file =>
...
for (const page of fs.readdirSync('.').filter(p => p.endsWith('.html'))) {
  ...
  const updated = html.replace(/((?:src|href)=")(energy\.js|app-shell\.js|app-shell\.css)(?:\?[^\"]*)?"/g,
