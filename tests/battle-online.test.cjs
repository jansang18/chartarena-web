...
test('actual hostStart transaction uses fresh capacity after the last human joins', async () => {
  const fresh = { status: 'wait_v7', rulesVersion:7,tableId:'standard', host: 'host', cap: 4, players: { host: {}, p2: {}, p3: {}, p4: {} } };
...
test('actual hostStart transaction cannot resurrect a cancelled room', async () => {
  ...
  assert.equal((await runHostStart({ status: 'wait_v7', rulesVersion:7,tableId:'standard', host: 'host', cap: 4, players: { p2: {} } })).length, 0);
});

test('host cannot start a different table or old rules version',async()=>{
  for(const extra of [{rulesVersion:4},{tableId:'expert'}]){
    const d={status:'wait_v7',rulesVersion:7,tableId:'standard',host:'host',cap:4,players:{host:{}},...extra};
...
test('candidate matching never joins a different rate or old rules',async()=>{
  ...
  const other={status:'wait_v7',rulesVersion:7,tableId:'expert',host:'host',createdAt:Date.now(),players:{host:{}}};
...
test('join transaction rechecks table after candidate changes',async()=>{
  const h=matchmakingHarness();h.ctx.startModeLive();
  const d={status:'wait_v7',rulesVersion:7,tableId:'standard',host:'host',createdAt:Date.now(),players:{host:{}}};
...
test('late created room is closed and removes only cancelled attempt membership', async () => {
  ...
  h.ctx.liveCancel();
  const ref = { data: { host: 'guest', status: 'wait_v7', rulesVersion:7,tableId:'standard', players: { guest: { matchAttempt: tag } }, scores: { guest: 0 } } };
...
test('cancellation after join commit but before its response removes late membership', async () => {
  ...
  h.ctx.startModeLive();
  const ref = { data: { host: 'host', status: 'wait_v7', rulesVersion:7,tableId:'standard', createdAt: Date.now(), cap: 4, players: { host: {} }, scores: {} } };
  ...
  assert.equal(ref.data.status, 'wait_v7', 'guest cleanup must not close the host room');
...
test('old cleanup cannot remove a newer attempt that joined the same room', async () => {
  ...
  const ref = { data: { host: 'host', status: 'wait_v7', rulesVersion:7,tableId:'standard', players: { guest: { matchAttempt: h.ctx.mmTag(h.ctx.mmAttempt) }, host: {} } } };
...
test('late snapshot and host polling responses cannot start a cancelled room', async () => {
  ...
  const ref = { data: { host: 'guest', status: 'wait_v7', rulesVersion:7,tableId:'standard', players: { guest: { matchAttempt: h.ctx.mmTag(h.ctx.mmAttempt) } } },
  ...
  pendingGet.resolve({ exists: true, data: () => ({ status: 'wait_v7', rulesVersion:7,tableId:'standard', cap: 1, players: { guest: {} } }) }); await flush();
