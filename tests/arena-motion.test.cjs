...
test('motion loads only on screen and returns to still when backgrounded or reduced',()=>{
 const f=fixture(); assert.match(f.img.src,/hyesu-hd-v2.png$/);
 f.visible(true);assert.match(f.img.src,/hyesu-idle-hd-v2.webp$/);
 f.doc.hidden=true;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-hd-v2.png$/);
 f.doc.hidden=false;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-idle-hd-v2.webp$/);
 f.media.matches=true;f.listeners.media();assert.match(f.img.src,/hyesu-hd-v2.png$/);
});
...
test('legacy manual pause cannot prevent default motion after returning to the page',()=>{
 ...
 f.doc.hidden=true;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-hd-v2.png$/);
 f.doc.hidden=false;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-idle-hd-v2.webp$/);
 f.visible(false);f.visible(true);assert.match(f.img.src,/hyesu-idle-hd-v2.webp$/);
});
