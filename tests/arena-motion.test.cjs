...
function fixture(paused='0'){
 ...
 const img={src:'assets/traders/hyesu-v2.png',isConnected:true,addEventListener:(n,f)=>listeners[n]=f,getAttribute(){return this.src;},setAttribute(n,v){this[n]=v;}};
...
test('motion loads only on screen and returns to still when backgrounded or reduced',()=>{
 const f=fixture(); assert.match(f.img.src,/hyesu-v2.png$/);
 f.visible(true);assert.match(f.img.src,/hyesu-idle-v2.webp$/);
 f.doc.hidden=true;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-v2.png$/);
 f.doc.hidden=false;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-idle-v2.webp$/);
 f.media.matches=true;f.listeners.media();assert.match(f.img.src,/hyesu-v2.png$/);
});
...
test('data saving does not request animated assets',()=>{
 const f=fixture();f.win.navigator.connection.saveData=true;f.visible(true);assert.match(f.img.src,/hyesu-v2.png$/);
});
test('legacy manual pause cannot prevent default motion after returning to the page',()=>{
 const f=fixture('1');f.visible(true);assert.match(f.img.src,/hyesu-idle-v2.webp$/);
 f.doc.hidden=true;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-v2.png$/);
 f.doc.hidden=false;f.listeners.visibilitychange();assert.match(f.img.src,/hyesu-idle-v2.webp$/);
 f.visible(false);f.visible(true);assert.match(f.img.src,/hyesu-idle-v2.webp$/);
});
