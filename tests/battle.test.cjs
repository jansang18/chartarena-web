...
function play(input,moves,reduced=false,decisions=['GO','GO']){
 ...
 const c={ArenaRules:R,players,matchHistory:[],MODE:'4p',LIVE:null,phase:'pick',tmr:null,revTimer:null,checkpointTimer:null,checkpointChoice:null,checkpointDeadline:0,revealStage:0,reduce:reduced,round:1,highlight:null,
...
test('missed entry and one intentional pass are free and do not require GO',()=>{const r=play(null,[1,2,10]),p=play({dir:'W'},[1,2,10]);assert.equal(r.c.players[0].state.balance,20000);assert.equal(r.c.players[0].state.passUsed,false);assert.equal(p.c.players[0].state.balance,20000);assert.equal(p.c.players[0].state.passUsed,true);});
